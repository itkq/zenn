...
[Configuration Options](https://docs.sentry.io/product/relay/options/) の通り、設定はファイルを用意して引数指定により行います。ソースコードを読むと、一部環境変数による指定が可能ですが、すべてではないため、ファイルの用意が無難です。ConfigMap を定義します。
